...
export function EmailTemplatesModal({ onClose }: EmailTemplatesModalProps) {
  ...
  const [templates, setTemplates] = useState<EmailTemplate[]>([]); // ✅ define templates state
  const [editableSubject, setEditableSubject] = useState(selectedTemplate?.subject || "");
  const [editableBody, setEditableBody] = useState(selectedTemplate?.body || "");
...
useEffect(() => {
  const fetchTemplates = async () => {
    ...
    try {
      const response = await axios.get("/api/auth/get-template", {
        headers: {
